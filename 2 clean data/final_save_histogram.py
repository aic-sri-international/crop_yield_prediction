...
class CropHistograms():
    ...
    ----------
    output_dir : str
        file path of directory with files of name <year>_<state_num>_<county_num>.npy, containing the numpy arrays
    ...
    def __init__(self, output_dir, data_yield_file, data_yield_subset_file, locations_file):
        ...
        output_dir : str
            file path of directory with files of name <year>_<state_num>_<county_num>.npy, containing the numpy arrays
            outputted by final_clean_data and the directory where histograms will be saved
        data_yield_file : str
            file path containing the csv info corresponding to the data yield for each location and year
        data_yield_subset_file : str
            file path where the new csv yields corresponding to the output_dir files will be stored
    ...
    def filter_timespan(self, image_temp, start_day, end_day, bands):
        ...
        if end_index > image_temp.shape[2]:
            image_temp = np.concatenate(
                (image_temp, np.zeros((image_temp.shape[0], image_temp.shape[1], end_index - image_temp.shape[2]))),
                axis=2)
    ...
    def build_and_save_histogram(self):
        ...
        for i in self.index_all:
            ...
            image_temp = self.filter_timespan(image_temp=image_temp, start_day=49, end_day=305, bands=9)
            ...
            image_temp = self.calc_histogram(image_temp=image_temp, bin_seq=bin_seq, bins=32, times=32, bands=9)
            image_temp[np.isnan(image_temp)] = 0
